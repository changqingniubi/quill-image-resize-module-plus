...
var Resize = function (_BaseModule) {
	...
	function Resize() {
		...
		return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Resize.__proto__ || Object.getPrototypeOf(Resize)).call.apply(_ref, [this].concat(args))), _this), _this.onCreate = function () {
			...
			_this.boxes = [];
			_this.imageMaxWidth = _this.options.imageMaxStyles.width;
			_this.imageMinWidth = _this.options.imageMinStyles.width;
			...
			if (_this.dragBox === _this.boxes[0] || _this.dragBox === _this.boxes[3]) {
				...
				var width = Math.round(_this.preDragWidth - deltaX);
				if (_this.preDragWidth > _this.imageMinWidth) {
					if (width >= _this.imageMinWidth && width <= _this.imageMaxWidth) {
						_this.img.width = width;
					}
				} else {
					if (width >= _this.preDragWidth && width <= _this.imageMaxWidth) {
						_this.img.width = width;
					}
				}
			} else {
				// right-side resize handler; dragging right enlarges image
				var _width = Math.round(_this.preDragWidth + deltaX);
				if (_this.preDragWidth > _this.imageMinWidth) {
					if (_width >= _this.imageMinWidth && _width <= _this.imageMaxWidth) {
						_this.img.width = _width;
					}
				} else {
					if (_width >= _this.preDragWidth && _width <= _this.imageMaxWidth) {
						_this.img.width = _width;
					}
				}
			}
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_parchment__ = __webpack_require__(105);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_quill_assets_icons_align_left_svg__ = __webpack_require__(107);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_quill_assets_icons_align_center_svg__ = __webpack_require__(106);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_quill_assets_icons_align_right_svg__ = __webpack_require__(108);
...
var FloatStyle = new __WEBPACK_IMPORTED_MODULE_0_parchment___default.a.Attributor.Style('float', 'float');
var MarginStyle = new __WEBPACK_IMPORTED_MODULE_0_parchment___default.a.Attributor.Style('margin', 'margin');
var DisplayStyle = new __WEBPACK_IMPORTED_MODULE_0_parchment___default.a.Attributor.Style('display', 'display');
	...
	this.checkImage = function (evt) {
		if (_this.img) {
			if (evt.keyCode == 46 || evt.keyCode == 8) {
				_this.quill.find(_this.img).deleteAt(0);
			}
			_this.hide();
		}
	};
